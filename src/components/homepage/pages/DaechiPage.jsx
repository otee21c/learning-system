...
export default function DaechiPage() {
  const [content, setContent] = useState('');
  const [title, setTitle] = useState('대치 연구소');
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const docRef = doc(db, 'pages', 'daechi');
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setTitle(docSnap.data().title || '대치 연구소');
          setContent(docSnap.data().content || '');
        }
      } catch (error) {
        console.error('Error fetching daechi:', error);
      }
      setLoading(false);
    };
    fetchContent();
  ...
  const handleSave = async () => {
    try {
      await setDoc(doc(db, 'pages', 'daechi'), {
        title: editTitle,
        content: editContent,
    ...
    <div className="homepage">
      ...
      <div className="hp-page-container">
        ...
        <div className="hp-page-content">
          <div className="hp-branch-detail">
            <div className="hp-branch-detail-header">
              <div className="hp-branch-detail-icon">
                <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12 3L1 9l4 2.18v6L12 21l7-3.82v-6l2-1.09V17h2V9L12 3zm6.82 6L12 12.72 5.18 9 12 5.28 18.82 9zM17 15.99l-5 2.73-5-2.73v-3.72L12 15l5-2.73v3.72z"/>
                </svg>
              </div>
              <div className="hp-branch-detail-info">
                <span className="hp-branch-badge">본원</span>
                <h2>오늘의 국어 연구소 대치</h2>
                <p>서울 강남구 대치동</p>
              </div>
            </div>
                ...
                <div className="hp-content-body">
                  {content ? (
                    ...
                    <div className="hp-default-content">
                      <p>오늘의 국어 연구소 본원입니다.</p>
                      <p>단 하나의 컨설팅 프로그램으로 실전적 국어 원리를 가르칩니다.</p>
                      <br />
